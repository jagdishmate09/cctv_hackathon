...
## 3. Agentic CCTV Pipeline (CLI)

This repo also includes an **agentic, cost-aware** CCTV pipeline under `src/`.
It supports:
- Multi-model YOLO (LOW/MED/HIGH)
- Motion-based YOLO skipping with a periodic "heartbeat"
- Occupancy persistence for seated people (prevents occupancy dropping to 0 when motion is low)
- Policy-based alerts for restricted rooms and meeting rooms
- All logs written to a single `logs/` folder

### 3.1 Install dependencies

From the project root (same venv is fine):

```bash
pip install -r requirements.txt
```

### 3.2 Configure camera policies

Edit `config/zones.yaml` and define each camera:
- `zone_type`: `RESTRICTED` or `UNRESTRICTED`
- `allowed_time` and `allowed_days` (for restricted)
- `allowed_occupancy` (capacity)
- `roi.polygon`: leave empty for full frame
...
python -m src.main \
  ...
  --min-yolo-interval-seconds 5 \
  --stale-occupancy-ttl-seconds 30
```
...
Logs are written to:
- `logs/metrics.jsonl` (time series)
- `logs/decisions.jsonl` (agent mode changes)
- `logs/alerts.jsonl` (alerts)
- `logs/run_summary.json` (run summary)
